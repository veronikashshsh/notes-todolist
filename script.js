...
function createNoteElement(title, content, section, date) {
...
noteForm.addEventListener("submit", function (e) {
  ...
  const noteForAll = createNoteElement(title, content, section, date);
  const noteForSection = createNoteElement(title, content, section, date);
...
function saveNotes() {
  ...
  allNotes.forEach(note => { // Проходимо по всіх нотатках
    const title = note.querySelector(".note-title strong").textContent; 
    ...
    notes.push({ title, content, section, date }); 
  });
...
function loadNotes() {
  ...
  notes.forEach(note => {
    const { title, content, section, date } = note;
